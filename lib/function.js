...
export default new (class Function {
         ...
         try {
            if (/^https?:\/\//i.test(string)) {
               ...
               resolve({
                  ...
                  name: /filename/i.test(data.headers?.get("content-disposition")) ? data.headers?.get("content-disposition")?.match(/filename=(.*)/)[1] : '',
   ...
   async resizeImage(buffer, height) {
      ...
      const resizeByMax = async (img, maxSize) => {
         ...
         const outputRatio =
            maxSize / Math.max(metadata.height, metadata.width);
         return img.resize(
   ...
   isUrl(url) {
      ...
      if (!regex.test(url)) throw "Input Is Not Url"
