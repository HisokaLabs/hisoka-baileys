...
const { jidNormalizedUser, proto, areJidsSameUser, extractMessageContent, downloadContentFromMessage, toBuffer } = baileys
import fs from "fs"
   ...
   const client = Object.defineProperties(hisoka, {
      ...
      sendMedia: {
         async value(jid, url, quoted = "", options = {}) {
            ...
            if (size >= 85000000) data = { document: buffer, mimetype: mime, fileName: options?.fileName ? options.fileName : `${hisoka.user?.name} (${new Date()}).${ext}`, ...options }
