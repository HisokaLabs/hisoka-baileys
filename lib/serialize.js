   ...
   const client = Object.defineProperties(hisoka, {
      ...
      sendMedia: {
         async value(jid, url, quoted = "", options = {}) {
            let { mime, data: buffer, ext } = await Function.getFile(url)
            mime = options?.mimetype ? options.mimetype : mime
            let data = { text: "" }, mimetype = /audio/i.test(mime) ? "audio/mpeg" : mime
            if (size >= 95000000) data = { document: buffer, mimetype: mime, fileName: options?.fileName ? options.fileName : `${hisoka.user?.name} (${new Date()}).${ext}`, ...options }
            if (options.asDocument) data = { document: buffer, mimetype: mime, fileName: options?.fileName ? options.fileName : `${hisoka.user?.name} (${new Date()}).${ext}`, ...options }
...
export async function Serialize(hisoka, msg) {
   ...
   if (m.message) {
      ...
      m.args = m.body.trim().replace(new RegExp(`^${m.prefix}`, 'i'), '').replace(m.command, '').split(/ +/).filter(a => a) || []
      m.text = m.args.join(" ")
   ...
   if (m.msg?.contextInfo?.quotedMessage) {
      ...
      if (m.quoted.message) {
         ...
         m.quoted.command = m.quoted.body && m.quoted.body.replace(m.quoted.prefix, '').trim().split(/ +/).shift().toLowerCase()
         m.quoted.arg = m.quoted.body.trim().split(/ +/).filter(a => a) || []
         m.quoted.args = m.quoted.body.trim().replace(new RegExp(`^${m.quoted.prefix}`, 'i'), '').replace(m.quoted.command, '').split(/ +/).filter(a => a) || []
